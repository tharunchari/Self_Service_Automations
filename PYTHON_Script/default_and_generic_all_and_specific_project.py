...
def fetch_repositories():
    ...
    if MODE == "specific" and SPECIFIC_REPO:
        try:
            org, prefix = SPECIFIC_REPO.split("/")
        except ValueError:
            print("❌ Invalid REPO format. Expected format: org/repo-prefix")
            return []
 
        print(f"📘 Mode: specific | Repo prefix: {prefix} | Org: {org}")
 
        while True:
            print(f"Fetching repos page {params['page']}...")
            response = requests.get(f"https://api.github.com/orgs/{org}/repos", headers=HEADERS, params=params)
        ...
        for repo in all_repos:
            if repo["name"].lower().startswith(prefix.lower()):
                filtered_repos.append(repo)
 
        print(f"✅ Matched repositories with prefix '{prefix}': {len(filtered_repos)}")
        return filtered_repos
 
    print("📘 Mode: all - Fetching all repositories")
    while True:
        print(f"Fetching repos page {params['page']}...")
    ...
    for repo in repos:
        ...
        while True:
            ...
            for alert in alerts:
                ...
                try:
                    loc_url = alert.get("locations_url")
                    if loc_url:
                        loc_res = requests.get(loc_url, headers=HEADERS)
                        if loc_res.status_code == 200:
                            loc_data = loc_res.json()
                            for loc in loc_data:
                                path = loc["details"].get("path", "")
                                line = loc["details"].get("start_line", "")
                                if path:
                                    locations.append(f"{path} (line {line})")
                        else:
                            print(f"⚠️ Failed to get locations for alert {alert_number} in {full_name}")
                except Exception as e:
                    print(f"❌ Exception while fetching locations for alert {alert_number}: {e}")
