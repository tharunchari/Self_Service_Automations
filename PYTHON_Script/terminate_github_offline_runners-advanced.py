...
def get_idle_or_offline_runners(matched_runners):
    ...
    for instance_id, runner in matched_runners:
        ...
        # Avoid termination for busy runners, or runners with multiple associations (extra safety)
        if status == "online" and busy:
            print(f"Skipping busy runner {runner_name} ({instance_id})")
            continue
        # Check for multiple repository associations (extra safety)
...
def main():
    ...
    if idle_or_offline:
        ...
        subject = "Terminated Idle/Offline GitHub Self-Hosted Runners"
        message = "The following instances were terminated due to being idle or offline for more than 5 minutes:\n\n"
