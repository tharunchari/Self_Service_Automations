...
def main():
    ...
    if instances:
        ...
        message_lines.append("\nMatched EC2 ID's and GitHub Runners (to be stopped)")

        message_lines.extend(matched_ids)
 
        message_lines.append("\nNot Matched EC2 ID's (to be stopped)")
        ...
        # Send mail only when EC2 > threshold exist

        send_sns_notification("EC2 & GitHub Runners Report", message_text)
 
        # Stop instances only when EC2 > threshold exist

        if matched_ids or not_matched_ids:

            print("\nEligible EC2 instances to stop (tagged 'Github_Self_Hosted_Runner'):")

            for inst in instances:

                print(f"  - {inst['id']} (Tags: {inst['tags']})")
 
            to_stop = matched_ids + not_matched_ids

            print(f"\nStopping EC2 instances now: {to_stop}")
            ...
            for inst_id in matched_ids:
                ...
                print(f"Stopped matched instance: {inst_id}")

            for inst_id in not_matched_ids:

                print(f"Stopped not matched instance: {inst_id}")

    else:

        print("No EC2 instances above threshold. Skipping SNS and stop operations.")
